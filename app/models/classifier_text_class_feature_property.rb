class ClassifierTextClassFeatureProperty < ActiveRecord::Base
  ...
  def self.import_to_naive_bayes(classifier_id)
    ...
    result_hash[:docs_count] = Hash[ Classifier.find(classifier_id).text_classes.collect{ |text_class| [ text_class.id, text_class.feeds.tagged_with(["dev_train", "was_trainer"], :any => true).count ] } ]
