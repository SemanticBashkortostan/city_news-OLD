...
class NbWihDict
  ...
  def run    
    Feed.tagged_with(["dev_train", "to_train", "was_trainer"], :any => true).where( :text_class_id => TextClass.all  ).each do |feed|
      @nb.train( feed.features_for_text_classifier, feed.text_class_id )
    end
    ...
    test_data  = Feed.tagged_with("dev_test").where( :text_class_id => cities ).all
