...
class CreateFeedSources < ActiveRecord::Migration
  def up
    ...
    sources = { TextClass.find_by_name("Ишимбай").try(:id) => ["http://ishimbay-news.ru/rss.xml", "http://ishimbai.procrb.ru/rss/?rss=y",
             ...
             TextClass.find_by_name("Салават").try(:id) => ["http://slvnews.ru/rss", "http://rssportal.ru/feed/163654.xml", "http://rosevronews.ru/feed/" ],
             TextClass.find_by_name("Уфа").try(:id) => ["http://rssportal.ru/feed/129727.xml", "http://news.yandex.ru/Ufa/index.rss"],
             TextClass.find_by_name("Стерлитамак").try(:id) => ["http://rssportal.ru/feed/223350.xml", "http://sterlegrad.ru/rss.xml", "http://cityopen.ru/?feed=rss2"],
             TextClass.find_by_name("Нефтекамск").try(:id) => ["http://neftekamsk.procrb.ru/rss/?rss=y", "http://rssportal.ru/feed/240378.xml",
    ...
    sources.each do |tc_id, urls|
      urls.each do |url|        
        p ["CreateFeedSources", FeedSource.create( :text_class_id => tc_id, :url => url)]
      end
    end 
